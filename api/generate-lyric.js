...
function enforceCleanOutput(text) {
  ...
  // 1) ⭐ 하이픈(-) 유지! Suno V5는 하이픈으로 롱톤 표현 (공식 권장)
  // out = out.replace(/-/g, ''); // ← 삭제 금지!
...
// =====================================
// 발라드 부스트 시스템 (v12.3 추가)
// =====================================
// 오빠의 취향을 발라드 계열에 자동 반영
const BALLAD_BOOST = `
Rich close harmonies with gentle counterpoint responses,
frequent melodic turns and unexpected harmonic changes,
late-onset slow steady vibrato on sustained notes,
one controlled powerful belt peak in the final chorus.`;

// 발라드 부스트가 적용될 장르 목록
const BALLAD_GENRES = ['pop80s', 'trot', 'ballad', 'acoustic_folk', 'rock_ballad', 'rnb'];

// ⭐ 조건별 프롬프트 버전 선택 함수
// - optionCount: 사용자가 활성화한 옵션 개수
// - genre: 선택된 장르
// 반환: 'short' 또는 'long'
function getPromptVersion(genre, optionCount, abModeActive) {
  // 발라드 계열 + 많은 옵션 활성화 = Long
  if (BALLAD_GENRES.includes(genre) && (optionCount >= 3 || abModeActive)) {
    return 'long';
  }
  
  // 발라드 계열일 때도 기본은 short (속도 중시)
  if (BALLAD_GENRES.includes(genre)) {
    return 'short';
  }
...
// ⭐ 발라드 부스트 적용 함수
// prompt에 오빠 취향 요소를 자동 추가
function applyBalladBoost(prompt, genre, shouldApplyBoost = true) {
  if (!shouldApplyBoost || !BALLAD_GENRES.includes(genre)) {
    return prompt;
  }
  
  // 프롬프트 끝에 부스트 추가 (기존 내용 유지)
  return prompt + '\n\n⭐ SIGNATURE SOUND (Oppa\'s choice):\n' + BALLAD_BOOST;
}

// 장르별 가사 공식 설정 + Short/Long 프롬프트
const GENRE_CONFIGS = {
  // 💎 감성 장인 팩 (발라드 계열)
  pop80s: {
    name: '80s 팝 발라드',
    ...
    promptShort: '80s melodic pop ballad, sorrowful, deep minor key, big singable hook, frequent melodic twists, lush analog synths, warm piano, rich close harmonies, late-onset slow steady vibrato, one high-note peak, no narration',
    promptLong: '80s melodic pop ballad, extremely sorrowful, deep minor key, medium-slow tempo, big singable hook, frequent emotional melodic twists and non-repetitive melodic contour, wide lush analog synth layers, warm piano, gated-reverb style drums kept elegant, rich layered close harmonies plus counterpoint backing lines in choruses, gradual build to a soaring final chorus with one big high-note peak, late-onset slow steady vibrato on long sustains, no narration'
  },
  trot: {
    name: '정통 트로트',
    ...
    promptShort: 'Traditional Korean Trot Ballad, slow to medium tempo, deep minor key, strong emotional kkeok-gi bend-and-release phrasing, heartfelt bittersweet nostalgia, rich backing vocals, acoustic guitar and warm piano, subtle strings swell, one clear peak moment, no narration',
    promptLong: 'Traditional Korean Trot Ballad, slow to medium tempo, deep minor key with clear singable melody, strong emotional bend-and-release phrasing (kkeok-gi) on long vowels, heartfelt bittersweet nostalgia, simple but powerful chord movement, rich backing vocals in the chorus with call-and-response, gentle acoustic guitar comping plus warm piano, subtle strings swelling toward the final chorus, one clear peak moment near the end, no narration'
  },
  ballad: {
    name: '한국형 감성 발라드',
    syllableRule: '3-6음절',
    finalChorusMax: 8,
    chorusLines: '5-6줄',
    special: '낮은 비브라토, 멜로디 중심',
    promptShort: 'Korean emotional ballad, medium-slow tempo, deep minor key, melody-first with frequent melodic turns, legato vocal phrasing with late-onset slow steady vibrato, rich layered close harmonies with gentle counterpoint, warm piano and legato strings, cathartic final chorus, clear evolving hook, no narration',
    promptLong: 'Korean emotional ballad, medium-slow tempo, deep minor key, melody-first with frequent obvious melodic turns (no monotonous contour), legato vocal phrasing with late-onset slow steady vibrato on sustained notes, rich layered close harmonies throughout with gentle counterpoint responses to the lead, warm piano and legato strings, gradual build to a cathartic final chorus with one strong belt peak, clear hook that evolves each chorus, no narration'
  },
  acoustic_folk: {
    name: '어쿠스틱 포크',
    ...
    promptShort: 'Acoustic folk, gentle healing mood but bittersweet, slow to medium tempo, intimate close-mic vocal, warm acoustic guitar picking and soft piano, simple memorable melody, light harmony doubles in chorus, natural storytelling tone, no narration',
    promptLong: 'Acoustic folk, gentle healing mood but bittersweet, slow to medium tempo, intimate close-mic vocal, warm acoustic guitar picking and soft piano, simple but memorable melody that subtly changes each section, light harmony doubles in chorus, small string or pad swell near the end, natural storytelling tone without talking, no narration'
  },
  rock_ballad: {
    name: '록 발라드',
    syllableRule: '3-6음절 (고음 록소 유도)',
    finalChorusMax: 9,
    chorusLines: '5-6줄',
    special: '절제된 힘, 고음 폭발은 Final Chorus에만 1회!',
    promptShort: 'Rock ballad, medium-slow tempo, deep minor key, strong verse-to-chorus lift with bigger chord tension, strong melodic hook, electric guitars build gradually, powerful chorus drums, rich backing harmonies, one high-note peak near final chorus, dramatic build and release, no narration',
    promptLong: 'Rock ballad, medium-slow tempo, deep minor key, clear verse-to-chorus lift with bigger chord tension, strong melodic hook, electric guitars build gradually (clean to driven), powerful drums in chorus, rich backing harmonies in chorus, one controlled explosive high-note peak only near final chorus, dramatic build and release, no narration'
  },
  rnb: {
    name: '한국형 R&B 발라드',
    syllableRule: '3-6음절',
    ...
    promptShort: 'Korean R&B Ballad, slow to medium tempo, deep minor key, smooth melodic line with tasteful runs at emotional points, stacked harmonies and ad-libs supporting the lead, call-and-response backgrounds in chorus, warm Rhodes / soft synth pads, restrained groove, cinematic string swells, emotional build to one big peak, no narration',
    promptLong: 'Korean R&B Ballad, slow to medium tempo, deep minor key, smooth melodic line with tasteful runs only at emotional points (not constant), stacked harmonies and ad-libs supporting the lead, call-and-response backgrounds in the chorus, warm Rhodes / soft synth pads, restrained groove with clean drums, cinematic string swells for the final lift, emotional build to one big peak in the final chorus, no narration'
  },
  kpop_dance: {
    name: 'K-Pop 아이돌 댄스',
    ...
    finalChorusMax: 12,
    chorusLines: '6-8줄',
    special: '중독성 후크 반복이 생명! "나나나", "예예예" 같은 캐치프레이즈 필수! 음절 제한 없이 리드미컬 요소!',
    promptShort: 'K-pop idol dance-pop, upbeat tempo, bright modern production, strong catchy hook, clear sectional contrast (verse / pre / chorus / bridge), layered harmony stacks in chorus, energetic ad-libs, punchy drums, synth leads, final chorus bigger with extra counter-melody, no narration',
    promptLong: 'K-pop idol dance-pop, upbeat tempo, bright modern production, strong melodic topline with catchy hook, clear sectional contrast (verse / pre / chorus / bridge), layered harmony stacks in chorus, energetic ad-libs near the end, punchy drums and tight bass, synth leads and ear-candy, final chorus bigger with extra counter-melody, no narration'
  },
  city_pop: {
    name: '시티 팝',
    ...
    promptShort: 'City pop, smooth mid-tempo groove, warm vintage keys, jazzy chords with bright yet wistful minor flavor, clean melodic hook, soft slap bass feel, crisp drums, shimmering guitars, layered chorus harmonies and subtle counter-melody, sunset-night drive vibe, no narration',
    promptLong: 'City pop, smooth mid-tempo groove, warm vintage keys, jazzy chords with bright yet wistful minor flavor, clean melodic hook, soft slap bass feel, crisp drums, shimmering guitars, layered chorus harmonies and subtle counter-melody lines, sunset-night drive vibe, no narration'
  },
  lofi_hiphop: {
    name: '로파이 힙합',
    ...
    finalChorusMax: 4,
    chorusLines: '2-3줄',
    special: '가사 거의 없음! 짧은 읊조림 또는 콧노래! "hmm...", "yeah..." 같은 최소 보컬! 분위기 중심!',
    promptShort: 'Lo-fi hip-hop, chill mid tempo, dusty drums and warm vinyl texture, mellow jazzy chords, simple short vocal hook lines, soft harmony pads, minimal emotional melody, late-night nostalgic mood, no narration',
    promptLong: 'Lo-fi hip-hop, chill mid tempo, dusty drums and warm vinyl texture, mellow jazzy chords, simple short vocal hook lines that feel sung not rapped, soft harmony pads and quiet backing responses, minimal but emotional melody, late-night nostalgic mood, no narration'
  },
  europop: {
    name: 'ABBA 스타일 유로팝',
    syllableRule: '3-5음절 (다음 화음 유도)',
    finalChorusMax: 9,
    chorusLines: '6-7줄',
    special: '사운드의 벽! 드라마틱한 전개와 풍부한 화음',
    promptShort: 'Melodic 70s Europop with deep minor key sadness but energetic drive, fast to mid-fast tempo, bright analog warmth, catchy hook with surprising chord changes, strong melodic motion, pulsing rhythmic foundation, shimmering synths, lush disco-style strings, layered backing vocals and harmonies, wide stereo feel, emotional yet danceable melancholy, no narration',
    promptLong: 'Melodic 70s Europop with deep minor key sadness but energetic drive, fast to mid-fast tempo, bright analog warmth, catchy hook with surprising chord changes, strong melodic motion and clear chorus lift, pulsing rhythmic foundation, shimmering synths, lush disco-style strings, layered backing vocals and harmonies in the chorus, wide stereo feel, emotional yet danceable melancholy, no narration'
  },
  metal: {
    name: '심포닉/고딕 메탈',
    syllableRule: '3-5음절',
    ...
    promptShort: 'Symphonic / Gothic Metal in deep minor key, dramatic melancholic atmosphere, mid tempo with powerful choruses, cinematic orchestral strings and choir layers supporting heavy guitars, clear heroic melody line (not growled), tight harmony stacks in chorus, call-and-response choir phrases, big final chorus with one clear peak note, modern polished production, no narration',
    promptLong: 'Symphonic / Gothic Metal in deep minor key, dramatic melancholic atmosphere, mid tempo with powerful choruses, cinematic orchestral strings and choir layers supporting heavy guitars, clear heroic melody line (not growled), tight harmony stacks in chorus, call-and-response choir phrases against lead vocal, big build into a crushing final chorus with one clear peak note, modern polished production, no narration'
  },
  hip_hop_boom_bap: {
    name: 'Hip-Hop Boom Bap',
    syllableRule: '7-12음절 (율동적)',
    finalChorusMax: 10,
    chorusLines: '4-6줄 (Hook)',
    special: 'End Rhyme 필수 (AABB/ABAB), Internal Rhyme 권장, 명확한 발음, 90s 골든 에이지 바이브',
    promptShort: 'Old school boom bap hip-hop, 90s golden-era vibe, steady head-nod tempo, clear enunciation, tight rhyme at line ends with internal rhymes, chorus is sung hook with harmony stacks and call-and-response, jazzy sample-like piano loop, deep bass, crisp kick/snare, no mumble delivery, no narration',
    promptLong: 'Old school boom bap hip-hop, 90s golden-era vibe, steady head-nod tempo, clear enunciation, tight rhyme at line ends with occasional internal rhymes, chorus is sung hook with harmony stacks and call-and-response, jazzy sample-like piano loop, deep bass, crisp kick/snare, no mumble delivery, no narration'
  },
  trap_melodic: {
    name: 'Melodic Trap',
    ...
    promptShort: 'Melodic trap, dark minor key, modern 808s and clean hats, melodic rap verses with strong sung chorus, emotional vulnerable theme, clear hook, tasteful harmony layers in chorus, subtle ad-libs, build into bigger final chorus with added counter-melody, no narration',
    promptLong: 'Melodic trap, dark minor key, modern 808s and clean hats, melodic rap verses with strong sung chorus, emotional vulnerable theme, clear hook, tasteful harmony layers in chorus, subtle ad-libs, build into a bigger final chorus with added counter-melody, no narration'
  },
  funk_pop: {
    name: 'Funk Pop',
    ...
    promptShort: 'Funk pop, upbeat groove, syncopated bassline, bright guitar chanks, brass stabs, energetic chorus with tight harmonies, playful call-and-response lines, clear melodic hook, punchy drums, final chorus bigger with extra harmony stack and rhythmic breaks, no narration',
    promptLong: 'Funk pop, upbeat groove, syncopated bassline, bright guitar chanks, brass stabs, energetic chorus with tight harmonies, playful call-and-response lines, clear melodic hook, punchy drums, final chorus bigger with extra harmony stack and rhythmic breaks, no narration'
  },
  reggaeton: {
    name: 'Reggaeton',
    syllableRule: '7-12음절 (라틴 스타일)',
    finalChorusMax: 8,
    chorusLines: '4-6줄 (Hook)',
    special: 'Dembow rhythm (3+3+2), Hook 최소 3회 반복 필수, 랩+노래 혼합, 라틴 바이브',
    promptShort: 'Reggaeton, dembow rhythm feel, warm club-ready production, catchy repetitive hook with sung chorus, Spanish-flavored ad-libs sparingly, verse-to-hook contrast, layered harmonies in hook, bass-driven groove, final hook bigger with extra harmony and counter line, no narration',
    promptLong: 'Reggaeton, dembow rhythm feel, warm club-ready production, catchy repetitive hook with sung chorus, Spanish-flavored ad-libs sparingly, verse-to-hook contrast, layered harmonies in hook, bass-driven groove, final hook bigger with extra harmony and counter line, no narration'
  },
  future_bass: {
    name: 'Future Bass',
    ...
    promptShort: 'Future bass, energetic tempo, bright uplifting chords with emotional minor moments, strong melodic topline, big chorus drop with vocal chops, layered harmony stacks, clear build-ups and releases, final chorus larger with added countermelody and extra harmonic layers, modern crisp production, no narration',
    promptLong: 'Future bass, energetic tempo, bright uplifting chords with emotional minor moments, strong melodic topline, big chorus drop with vocal chops, layered harmony stacks in chorus, clear build-ups and releases, final chorus larger with added countermelody and extra harmonic layers, modern crisp production, no narration'
  },
  indie_pop: {
    ...
    promptShort: 'Indie pop / bedroom pop, mid tempo, intimate vocal, simple but memorable melody with subtle unexpected turns, lo-fi warm guitars and soft synth pads, light harmony doubles in chorus, emotionally honest tone, final chorus adds gentle counter-melody and thicker harmonies, no narration',
    promptLong: 'Indie pop / bedroom pop, mid tempo, intimate vocal, simple but memorable melody with subtle unexpected turns, lo-fi warm guitars and soft synth pads, light harmony doubles in chorus, emotionally honest tone, final chorus adds a gentle counter-melody and thicker harmonies, no narration'
  }
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ⭐⭐⭐ SHORT/LONG 프롬프트 버전 자동 선택 (v12.3 신기능!) ⭐⭐⭐
    // 사용자 옵션이 많을수록, 또는 발라드 계열이면 Long 프롬프트 사용
    const optionCount = [vocalModifier, genreFusionModifier, vocalConfigModifier, emotionIntensityModifier, moodModifier].filter(Boolean).length;
    const promptVersion = getPromptVersion(genre, optionCount, abModeActive);
    const stylePromptKey = `prompt${promptVersion.charAt(0).toUpperCase() + promptVersion.slice(1)}`; // 'promptShort' 또는 'promptLong'
    const stylePrompt = GENRE_CONFIGS[genre]?.[stylePromptKey] || '';
    
    // 스타일 프롬프트 + 메인 프롬프트 결합
    if (stylePrompt) {
      lyricsPrompt = `🎵 SUNO STYLE (${promptVersion.toUpperCase()}):\n${stylePrompt}\n\n━━━━━━━━━━━━━━━━━━━━\n\n` + lyricsPrompt;
    }
    ...
    const payload = {
      ...
      generationConfig: {
        temperature: getTemperature(genre), // ⭐ 장르별 최적 온도 (v11.8)
        topK: 40,
        topP: 0.95,
        maxOutputTokens: getMaxOutputTokens(genre, structure, abModeActive) // ⭐ 장르/구조별 자동 조절
      }
    };
...
2️⃣ **STRUCTURE & LINE LIMITS** (${timeline.info} @ 72-84 BPM):
   ...
   🔥🔥🔥 YOU MUST GENERATE ALL SECTIONS BELOW - NO EXCEPTIONS! 🔥🔥🔥
   
   **⭐ COMPLETE SONG STRUCTURE (EVERY SECTION MANDATORY!):**
   ✅ [Intro] (${timeline.intro}): 2 lines
   ✅ [Verse 1] (${timeline.v1}): **4 lines ONLY**
   ✅ [Pre-Chorus] (${timeline.pre1}): **2 lines ONLY**
   ✅ [Chorus] (${timeline.c1}): **${config.chorusLines}**
   ...
   ✅ [Chorus] (${timeline.c2}): **${config.chorusLines}**
   ✅✅✅ **[Bridge]** (${timeline.bridge}): **4 lines** ← DO NOT SKIP THIS SECTION!
   ✅ [Instrumental Build] (${timeline.build}): (no lyrics)
   ✅✅✅ **[Key change up]** ← MANDATORY TAG! Write this tag exactly!
   ✅✅✅ **[Final Chorus]** (${timeline.final}): **${config.finalChorusMax} lines MAX!** ← DO NOT SKIP! (${config.finalChorusMax + 1}+ = guaranteed rap!)
   ✅ [Outro] (${timeline.outro}): 2 lines
...
4️⃣ **MELODY VARIATION TECHNIQUES**:
   ...
   c) **Consonant Ending Strategy**: 
      - Soft endings (ㄴ/ㅇ) for sustained notes
      - Hard endings (ㅁ/ㄱ/ㄷ) for abrupt stops
   d) **Tilde Placement**: Irregular distribution across lines
   e) **Ellipsis Usage** (말줄임표):
      - Max 0~2 per section
      - Use for emotional pause, hesitation
      - Examples: "그리 사랑일 걸...", "남아서..."
      - ⚠️ Too many = feels choppy and amateur!

5️⃣ **LONG NOTE HYPHEN** (Suno V5 Official Method):
   - Use **hyphen + vowel repetition** for sustained notes
   - **Open vowels = BEST for long notes!** (아/오/우/이)
   - Distribution: Verse (minimal) → Chorus (moderate) → Final Chorus (heavy)
   - Examples: "떨려-어" (lo-ove style), "돌아-아", "차가워-어", "흘러-어"
   - Alternative: "사랑해-애-애" (multiple vowels for extra length)
   - Max 2-3 per section
   
   ⚠️ **HYPHEN AND COUNTERPOINT COEXIST**: 
   - Hyphen (-) and English ad-lib ( ) CAN be on the same line (unlike ~)
   - ✅ ALLOWED: "웃던 장면-면 (only you)" → Works perfectly!
   - ✅ ALLOWED: "차가운 밤-암 (cold night)" → No conflict!
...
6️⃣ **HARMONY & COUNTERPOINT** (MANDATORY):
   ...
   - ⚠️ **Lines with counterpoint ( ) and hyphen (-) work together!**
   
   🚨🚨🚨 COUNTERPOINT CRITICAL RULES 🚨🚨🚨
   
   **RULE 1: SAME LINE ONLY** (NEVER new line!)
   ❌ WRONG - Counterpoint on new line:
   찰나의 조각
   뒤틀린 환영
   (Only shadows dancing in the dark)  ← NEW LINE = FORBIDDEN!
   
   ✅ CORRECT - Counterpoint on same line:
   찰나의 조각
   뒤틀린 환영 (shadows)  ← SAME LINE!
   기괴한 빛깔-알
   
   **RULE 2: LENGTH MATCHING** (Korean length = English length!)
   - Korean 4음절 → English 2-3 words (~4 syllables)
   - Examples:
     * "뒤틀린 환영" (4음절) → (twisted dreams) ✅
     * "찰나의 조각" (4음절) → (fading light) ✅
     * "기괴한 빛깔" (4음절) → (strange glow) ✅
   
   ❌ TOO LONG - 7+ words forbidden:
   - "Only shadows dancing in the dark" (7 words) ← TOO LONG!
   - Suno V5 won't harmonize properly with long counterpoint!
   
   ✅ CORRECT LENGTH - Max 3 words:
   - (shadows) ✅
   - (twisted dreams) ✅
   - (fading light) ✅
   - (hold me tight) ✅
...
7️⃣ **LITERARY TONE** (No direct emotion words):
   ...
   - Examples: "차가운 속도", "흐린 창문", "빈 의자", "시든 꽃잎"
...
🌟 **OUTPUT FORMAT**:
   ...
   **FORBIDDEN in lyrics:**
   ...
   ⭐ Hyphens "-" for long notes! (Suno V5 Official: "사랑-앙", "떨려-어")

   **CORRECT long note notation:**
   **CORRECT long note notation (Suno V5 Official):**
   ✅ "떨려-어" (hyphen + vowel repetition)
   ✅ "돌아-아" (hyphen + vowel repetition)
   ✅ "사랑해-애-애" (multiple vowels for extra length)

   **INCORRECT notation:**
   ❌ "떨려~" (tilde not officially supported in V5!)
   ❌ "돌아~" (use hyphen instead!)
...
□ ⭐ [Bridge] section included? (MANDATORY!)
□ ⭐ [Key change up] tag included? (MANDATORY!)
□ ⭐ [Final Chorus] section included? (MANDATORY!)
...
□ Hyphens (-) for long notes? (vowel repetition: "사랑-앙")
□ Ellipsis (...) max 0~2 per section?
□ Counterpoint ${abModeActive ? '2-3' : '1-2'} times per chorus?
□ ⭐ Counterpoint on SAME LINE (NOT new line)?
□ ⭐ Counterpoint length max 3 words? (NO 7+ word phrases!)
...
🎵 Generate the COMPLETE song NOW! MUST include ALL sections: [Intro] → [Verse 1] → [Pre-Chorus] → [Chorus] → ... → [Bridge] → [Instrumental Build] → [Key change up] → [Final Chorus] → [Outro]!

${BALLAD_GENRES.includes(genre) ? `
⭐ SIGNATURE SOUND (Oppa's special touch - automatically applied to ballad genres):
${BALLAD_BOOST}` : ""}
  `;
}
...
1️⃣ **RHYME SCHEMES** (Most important!):
   ...
   - Examples:
     * AABB: "거리 위를 걸어 / 꿈을 향해 더 / 밤하늘 별이 빛나 / 내 길을 비춰 봐"
