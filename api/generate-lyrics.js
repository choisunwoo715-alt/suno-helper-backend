...
// =====================================
// Ã­â€ Â Ã­ÂÂ° Ã¬Å¾ÂÃ«Ââ„¢ Ã¬Â¡Â°Ã¬Â Ë† Ã­â€¢Â¨Ã¬Ë†Ëœ (v11.3 Ã¬Â¶â€ÃªÂ°â‚¬)
// =====================================
function getMaxOutputTokens(genre, structure) {
  // lo-fiÃ«Å â€ ÃªÂ°â‚¬Ã¬â€šÂ¬ÃªÂ°â‚¬ ÃªÂ±Â°Ã¬ÂËœ Ã¬â€”â€ Ã¬Å“Â¼Ã«â€¹Ë† Ã­ÂÂ¬ÃªÂ²Å’ Ã¬Â¤â€žÃ¬â€”Â¬Ã«Ââ€ž Ã«ÂÂ¨
  if (genre === 'lofi_hiphop') return 900;
  
  // future_bassÃ«Ââ€ž Ã¬Â§Â§ÃªÂ²Å’
  if (genre === 'future_bass') return 1500;
  
  // ÃªÂµÂ¬Ã¬Â¡Â°Ã«Â³â€žÃ«Â¡Å“ Ã«Å’â‚¬Ã«Å¾Âµ Ã¬Â»Â· (Ã«Â¹â€žÃ¬Å¡Â© Ã¬Â Ë†ÃªÂ°Â)
  ...
  try {
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        topK: 40,
        topP: 0.95,
        maxOutputTokens: getMaxOutputTokens(genre, structure) // Ã¢Â­Â Ã¬Å¾Â¥Ã«Â¥Â´/ÃªÂµÂ¬Ã¬Â¡Â°Ã«Â³â€ž Ã¬Å¾ÂÃ«Ââ„¢ Ã¬Â¡Â°Ã¬Â Ë†
      }
    };
...
5Ã¯Â¸ÂÃ¢Æ’Â£ **LONG NOTE HYPHEN** (Suno V5 Official Method):
   ...
   🚨 **CRITICAL: HYPHEN-PARENTHESIS SPACING RULE** 🚨
   - **NEVER put parenthesis immediately after hyphen without vowel repetition!**
   - ❌ WRONG: "사랑한다- (love you)" → Hyphen directly before parenthesis = Harmony BREAKS!
   - ✅ CORRECT: "사랑한다-아 (love you)" → Hyphen + vowel + space + parenthesis = Perfect!
   - **Why:** Suno V5 needs clear separation between long-note signal (-) and harmony signal ( )
   - **Rule:** Always use vowel repetition after hyphen BEFORE adding counterpoint parentheses
...
6Ã¯Â¸ÂÃ¢Æ’Â£ **HARMONY & COUNTERPOINT** (MANDATORY):
   ...
   ðŸš¨ **RULE 3: NO CONSECUTIVE COUNTERPOINT** (CRITICAL!)
   âŒ FORBIDDEN - Two consecutive lines with counterpoint:
   ì›ƒë˜ ì–¼êµ´ (only you)    â† Line 1 has counterpoint
   ì°¨ê°€ìš´ ë°¤ (hold me)      â† Line 2 has counterpoint â†' FORBIDDEN!
   
   âœ… CORRECT - Skip at least one line between counterpoint:
   ì›ƒë˜ ì–¼êµ´ (only you)    â† Line 1 counterpoint
   ì°¨ê°€ìš´ ë°¤               â† Line 2 NO counterpoint
   ê¸°ì–µ ì†ì— (shadows)     â† Line 3 counterpoint â†' OK!
   
   **Why:** Suno V5 won't harmonize consecutive counterpoint lines - harmony breaks!
   **Rule:** Counterpoint MUST have irregular spacing (1+3, 1+4, 2+4, 2+5, etc.)
            NEVER put counterpoint on consecutive lines (1+2, 2+3, 3+4)!
   **This is CRITICAL** - consecutive counterpoint = NO harmony = waste of credits!
