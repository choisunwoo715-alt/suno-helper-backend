...
// ⭐ v12.3: Short/Long 분기 제거 - promptLong으로 통일!
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ⭐ v12.3: promptLong으로 통일! (간단하고 항상 디테일)
    let stylePrompt = GENRE_CONFIGS[genre]?.promptLong || '';
    
    // v13.0: BPM 템포 교체!
    if (bpmTempo && stylePrompt) {
      stylePrompt = stylePrompt.replace(/medium-slow tempo/gi, `at ${bpmTempo}`);
      console.log(`템포 교체: "medium-slow tempo" -> "${bpmTempo}"`);
    }
    
    // 스타일 프롬프트 + 메인 프롬프트 결합
    if (stylePrompt) {
      lyricsPrompt = `🎵 SUNO STYLE (LONG):\n${stylePrompt}\n\n┏━━━━━━━━━━━━━━━━━━━┓\n\n` + lyricsPrompt;
    }
...
4️⃣ **MELODY VARIATION TECHNIQUES**:
   ...
   c) **Consonant Ending Strategy**: 
      ...
      - Hard endings (ㅁ/ㅂ/ㄱ) for abrupt stops
...
7️⃣ **LITERARY TONE** (No direct emotion words):
   ...
   - Examples: "차가운 손도", "흐린 창문", "빈 의자", "시든 꽃잎"
