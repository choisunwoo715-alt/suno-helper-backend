...
// 백엔드 전용 - Vercel KV 레이트리밋 + 보안 강화 + 하이픈 고정판!
// =====================================================
...
function getMaxOutputTokens(genre, structure) {
  // lo-fi는 가사가 거의 없으니 적당히
  if (genre === 'lofi_hiphop') return 1000; // 900 → 1000 (안전 여유)
  
  // future_bass도 짧지만 여유 확보
  if (genre === 'future_bass') return 1800; // 1500 → 1800
  
  // 구조별로 안전하게 상향 (Final Chorus 잘림 방지!)
  if (structure === 'short') return 2500;    // 2200 → 2500
  if (structure === 'extended') return 4500; // 3800 → 4500 (완전 안전!)
  
  // standard (기본)
  return 3500; // 3000 → 3500 (여유 확보)
}
...
module.exports = async (req, res) => {
  ...
  const allowedOrigins = [
    ...
    'https://suno-helper.com',  // 오빠 커스텀 도메인!
...
6️⃣ **HYPHENS** (Moderate use - Suno V5 official!):
   - Use hyphen (-) for smooth sustained notes
...
8ï¸âƒ£ **OUTPUT FORMAT**:
   ...
   - ✅ Use hyphens (-) for smooth sustained notes (Suno V5 official!)
...
â–¡ Smooth, jazzy feel?
â–¡ Chorus variation?
□ Hyphens (-) for smooth notes? (Suno V5 official!)
...
5ï¸âƒ£ **LONG NOTE HYPHEN** (Suno V5 Official Method):
...
□ Melody variation applied? (음절수/구조/받침/하이픈-)
...
□ Ad-libs 2 lines max?
□ ⚠️ CRITICAL: Hyphen (-) and counterpoint ( ) CAN coexist on same line!
