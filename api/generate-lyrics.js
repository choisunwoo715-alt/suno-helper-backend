...
const MAX_IP_ENTRIES = 1000; // ⭐ 최대 IP 개수 제한
const CLEANUP_INTERVAL = 3600000; // 1시간마다 정리

// 메모리 누수 방지: 오래된 IP 자동 정리
function cleanupOldEntries() {
  const now = Date.now();
  const entriesToDelete = [];
  
  for (const [ip, requests] of rateLimitMap.entries()) {
    // 1시간 지난 요청만 남은 IP는 삭제 대상
  ...
  // 그래도 너무 많으면 오래된 순으로 삭제
  if (rateLimitMap.size > MAX_IP_ENTRIES) {
    const sortedEntries = Array.from(rateLimitMap.entries())
      .sort((a, b) => Math.min(...a[1]) - Math.min(...b[1])); // 가장 오래된 요청 기준
    
    const deleteCount = rateLimitMap.size - MAX_IP_ENTRIES;
    for (let i = 0; i < deleteCount; i++) {
      rateLimitMap.delete(sortedEntries[i][0]);
    }
  }
  
  console.log(`[Cleanup] Rate limit map size: ${rateLimitMap.size}`);
}
...
function checkRateLimit(ip) {
  ...
  // ⭐ Map이 너무 커지면 즉시 정리
  if (rateLimitMap.size > MAX_IP_ENTRIES * 1.2) {
    cleanupOldEntries();
  }
}
...
function validateInput(body) {
  ...
  modifierFields.forEach(field => {
    if (body[field] && typeof body[field] === 'string' && body[field].length > 200) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Rate Limiting 체크 (IP 추출 개선 v11.2)
...
① 한글: 3~5음절 / English: 8~12음절 (언어별 다름!)
...
1️⃣ **SYLLABLE RULE** (CRITICAL - Language-specific!):
   ...
   🇰🇷 **KOREAN LYRICS** (3~5 syllables - STRICT!):
   - **Verse: 3~5 syllables** (6+ = rap delivery!)
   - **Pre-Chorus: 3~4 syllables** (shorter = tension build!)
   - **Chorus: 3~5 syllables** (max 6)
   - **Bridge: 3~5 syllables**
   - **Final Chorus: 3~5 syllables**
   - **Outro: 2~4 syllables** (shorter = emotional fade)
   - ⚠️ Korean: 6+ syllables = GUARANTEED rap delivery!
   ...
   **English Examples:**
   ...
   - **V4.5+**: More flexible, sometimes handles 6-7 Korean syllables gracefully (random)
...
═══════════════════════════════════════════════════════════════
□ Language identified? (Korean = 3~5 / English = 8~12 syllables)
□ Korean: All lines 3~5음절? (Pre 3~4, Outro 2~4)
